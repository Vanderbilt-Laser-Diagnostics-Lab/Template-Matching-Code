...
[SI BC] = pointLocation(DT, XDel(:,1:2));
...
%     set = unique(TRI(rows,:));
    S = vertexAttachments(DT,i);
    set = unique(DT.Triangulation(cat(2,S{:}),:));
...
%     BNetsV{i} = PowellSabin(p1',p2',p3',d1V,d2V,d3V);
    ...
    p1 = XDel(DT.Triangulation(i,1),1:2);
    p2 = XDel(DT.Triangulation(i,2),1:2);
    p3 = XDel(DT.Triangulation(i,3),1:2);
    
    Nodes{i}=[p1; p2; p3];
    
    d1U = UDat(DT.Triangulation(i,1),:)';
    d2U = UDat(DT.Triangulation(i,2),:)';
    d3U = UDat(DT.Triangulation(i,3),:)';
    BNetsU{i} = PowellSabin(p1',p2',p3',d1U,d2U,d3U);
    
    d1V = VDat(DT.Triangulation(i,1),:)';
    d2V = VDat(DT.Triangulation(i,2),:)';
    d3V = VDat(DT.Triangulation(i,3),:)';
    BNetsV{i} = PowellSabin(p1',p2',p3',d1V,d2V,d3V);
end
...
SDT = TriRep(SubTris,P(:,1:2));

% T = tsearch( P(:,1),P(:,2),SubTris,pnts(:,1),pnts(:,2) );
S = pointLocation(SDT, pnts(:,1:2));
